        ...
        const Usuarios: React.FC = () => {
            ...
            const handleSubmit = async (e: React.FormEvent) => {
                ...
                try {
                    ...
                    if (formData.existsInAuth) {
                        ...
                        const createData: CreateUserData = { name: formData.name, email, password: formData.password, role: formData.role, allowedDeviceSerial: formData.allowedDeviceSerial };
                    ...
                    console.error(error);
                    if (error.code === 'auth/email-already-in-use' || (error.message && error.message.includes('email-already-in-use'))) setMessage('Erro: Este nome de usuário já está em uso. Tente outro.');
                    else setMessage('Erro ao criar usuário: ' + error.message);
                } finally {
                    setLoading(false);
                }
            };
            ...
            return (
                <div className="p-6">
                    ...
                    {!isAdmin && (
                        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded mb-6">Apenas administradores podem criar e sincronizar usuários. Solicite acesso ao administrador do sistema.</div>
                    )}
                                        ...
                                        {source === 'auth' ? (
                                            ...
                                            users.map((user) => (
                                                <tr key={user.uid} className="bg-white border-b hover:bg-gray-50">
                                                    ...
                                                    <td className="px-4 py-3 text-xs font-mono text-gray-400">{user.allowedDeviceSerial ? user.allowedDeviceSerial.substring(0, 8) + '...' : '-'}</td>
                                                </tr>
                                            ))
                                        )}
                                    </tbody>
