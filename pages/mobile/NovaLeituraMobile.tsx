...
import { compressImage, isValidImage, isValidSize } from '../../utils/imageCompressor';
...
const NovaLeituraMobile: React.FC = () => {
  ...
  useEffect(() => {
    const loadOperadoresDoPonto = async () => {
      if (!pontoId || !userProfile?.allowedLocalidades?.[0]) {
      ...
      try {
        ...
        // ⚠️ Query simplificada: apenas data + pontoId (sem múltiplos where para evitar índice)
        console.log(`🔎 BUSCANDO vendas com data='${hojeStr}' E pontoId='${pontoId}'`);
        ...
        console.log('🔍 QUERY RESULTADO:', { 
          ...
          pontoId, 
          localidadeId,
          totalDocsFull: vendasSnap.docs.length,
          docs: vendasSnap.docs.map(d => {
            const v = d.data();
            return { 
              data: v.data,
              dataType: typeof v.data,
              pontoId: v.pontoId, 
              localidadeId: v.localidadeId,
        ...
        // Filtrar manualmente por localidadeId e active
        const operadoresLidosHoje = vendasSnap.docs
          .filter(doc => {
            const venda = doc.data();
            return venda.localidadeId === localidadeId && venda.active === true;
          })
          .map(doc => doc.data().operadorId);

        console.log('✅ OPERADORES JÁ LIDOS:', operadoresLidosHoje);
        console.log('📋 OPERADORES DO PONTO:', operadoresDoPonto.map(o => ({ id: o.id, codigo: o.codigo, nome: o.nome })));

        // Filtrar operadores não lidos hoje
        const operadoresPendentes = operadoresDoPonto.filter(
          op => {
            const jaLido = operadoresLidosHoje.includes(op.id);
            console.log(`  - ${op.codigo} (${op.id.substring(0,8)}...): ${jaLido ? '❌ JÁ LIDO' : '✅ PENDENTE'}`);
            return !jaLido;
          }
        );

        console.log(`🎯 RESULTADO: ${operadoresPendentes.length} operadores pendentes de ${operadoresDoPonto.length} total`);

        // Se todos já foram lidos, deixar vazio (não liberar todos)
        setOperadoresFiltrados(operadoresPendentes);
      } catch (error) {
        console.error('Erro ao filtrar operadores:', error);
        // Em caso de erro, liberar operadores do ponto para não travar
        const operadoresDoPonto = operadores.filter(op => op.pontoId === pontoId);
        setOperadoresFiltrados(operadoresDoPonto);
      }
    };

    loadOperadoresDoPonto();
  }, [pontoId, operadores, userProfile]);
  ...
  const handleFotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isValidImage(file)) {
      setMensagem('❌ Formato inválido! Use apenas JPG, PNG ou WEBP.');
    ...
    if (!isValidSize(file, 10)) {
      setMensagem('❌ Imagem muito grande! Máximo 10MB.');
      setMessageType('error');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload da foto (comprimida) para Storage
      let fotoUrl = '';
      if (foto) {
        const storageRef = ref(storage, `leituras/${userProfile?.uid || 'anon'}/${Date.now()}-${foto.name}`);
        const uploadSnapshot = await uploadBytes(storageRef, foto);
        fotoUrl = await getDownloadURL(uploadSnapshot.ref);
        console.log('✅ Foto enviada para Storage:', { fotoUrl, tamanhoKB: (foto.size / 1024).toFixed(2) });
      }

      // Salvar leitura no Firestore
      const vendaData = {
        // 'data' vem do input como string YYYY-MM-DD no fuso local - usar direto sem conversão
        data: data, // Já é string YYYY-MM-DD, sem conversão
      ...
      console.log('📝 VENDA A SALVAR - Campos críticos:', { 
        data: vendaData.data, 
        pontoId: vendaData.pontoId,
        localidadeId: vendaData.localidadeId,
        active: vendaData.active,
        operadorId: vendaData.operadorId,
        timestamp: vendaData.timestamp?.toDate?.()
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/30 p-2 pb-20">
      <form onSubmit={handleSubmit} className="space-y-2 max-w-md mx-auto">
        ...
        <div className="text-center py-2">
          ...
          <label
            htmlFor="upload-foto"
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border-2 border-blue-300 bg-blue-50 text-blue-700 text-xs font-semibold cursor-pointer hover:bg-blue-100 hover:border-blue-400 transition"
          >
            <Upload size={14} />
            {foto ? 'Alterar foto (comprimida)' : 'Adicionar foto (comprimida)'}
          </label>
          {foto && (
          ...
          {!foto && (
            <p className="text-[10px] text-gray-400 mt-1">
              Será comprimida para ~200KB / 1024px
            </p>
          )}
        </div>
