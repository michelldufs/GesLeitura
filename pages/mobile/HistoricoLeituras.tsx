...
const HistoricoLeituras: React.FC = () => {
  ...
  const loadVendas = async () => {
    ...
    try {
      ...
      switch (filtroData) {
        case 'hoje':
          dataInicioStr = getTodayDateString();
          dataFimStr = getTodayDateString();
          break;
        case 'ontem':
          const ontem = new Date();
          ontem.setDate(ontem.getDate() - 1);
          dataInicioStr = formatDateToString(ontem);
          dataFimStr = formatDateToString(ontem);
          break;
        case 'semana':
          const semanaAtras = new Date();
          semanaAtras.setDate(semanaAtras.getDate() - 7);
          dataInicioStr = formatDateToString(semanaAtras);
          dataFimStr = getTodayDateString();
          break;
        case 'periodo':
      ...
      console.log('📅 Período (datas como strings):', { filtroData, dataInicioStr, dataFimStr });

      const q = query(
        collection(db, 'vendas'),
        where('coletorId', '==', userProfile.uid),
        where('localidadeId', '==', userProfile.allowedLocalidades?.[0] || ''),
        where('active', '==', true)
      );
      ...
      // Filtrar no cliente por data (comparar strings YYYY-MM-DD)
      let data = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() } as Venda))
        .filter(venda => {
          const vendaDataStr = venda.data; // Campo 'data' é string YYYY-MM-DD
          if (!vendaDataStr) {
            console.warn('⚠️ Venda sem campo "data":', venda);
            return false;
          }
          
          // Comparação de strings funciona para formato YYYY-MM-DD
          const match = vendaDataStr >= dataInicioStr && vendaDataStr <= dataFimStr;
          if (filtroData === 'ontem' && match) {
            console.log(`✓ Encontrada venda de ontem:`, { operadorId: venda.operadorId, data: vendaDataStr });
          }
          return match;
        });
      
      // Ordenar no cliente por timestamp (descrescente)
      data.sort((a, b) => {
