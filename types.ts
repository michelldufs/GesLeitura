...
export interface DespesaGeral {
  id?: string;
  data: string; // Data de criação/lançamento
  valor: number;
  descricao: string;
  userId: string;
  userName?: string;
  ...
  active: boolean;
  // Novos campos para contas a pagar/parcelamento
  status?: 'pago' | 'agendado';
  dataVencimento?: string; // ISO string (data prevista para pagamento)
  dataPagamento?: string; // ISO string (data real do pagamento)
  parcelaAtual?: number;
  parcelaTotal?: number;
  groupId?: string;
}
