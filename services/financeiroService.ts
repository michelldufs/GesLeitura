...
/**
 * Salva uma despesa simples ou parcelada/provisionada.
 * @param despesa Dados da despesa (pode conter info de parcelamento)
 * @param userId Usuário logado
 * @param options.extra Parcelamento: { parcelas, dataPrimeiroVencimento, primeiraPaga }
 */
export const saveDespesa = async (
  despesa: Omit<DespesaGeral, 'id'>,
  userId: string,
  options?: {
    parcelas?: number;
    dataPrimeiroVencimento?: string; // ISO
    primeiraPaga?: boolean;
    frequencia?: 'mensal' | 'semanal' | 'quinzenal';
    dataVencimentoAvulsa?: string; // Para despesa não parcelada
  }
) => {
  const parcelas = options?.parcelas && options?.parcelas > 1 ? options.parcelas : 1;
  const dataPrimeiroVenc = options?.dataPrimeiroVencimento || despesa.data;
  const primeiraPaga = !!options?.primeiraPaga;
  const frequencia = options?.frequencia || 'mensal';
  const ids: string[] = [];

  // Despesa não parcelada, mas agendada para o futuro
  if (parcelas === 1 && options?.dataVencimentoAvulsa) {
    const dataVencimento = options.dataVencimentoAvulsa;
    let status: 'pago' | 'agendado' = 'agendado';
    ...
    if (status === 'pago') {
      await verificarPeriodoFechado(dataPagamento || dataVencimento, despesa.localidadeId);
    }
  ...
  // Parcelada
  const groupId = parcelas > 1 ? uuidv4() : undefined;
  for (let i = 1; i <= parcelas; i++) {
    // Calcula data de vencimento da parcela conforme frequência
    let dataVencimento: string;
    if (frequencia === 'mensal') {
      dataVencimento = formatISO(addMonths(new Date(dataPrimeiroVenc), i - 1), { representation: 'date' });
    } else if (frequencia === 'semanal') {
      dataVencimento = formatISO(addDays(new Date(dataPrimeiroVenc), 7 * (i - 1)), { representation: 'date' });
    } else if (frequencia === 'quinzenal') {
      dataVencimento = formatISO(addDays(new Date(dataPrimeiroVenc), 15 * (i - 1)), { representation: 'date' });
    } else {
      dataVencimento = formatISO(addMonths(new Date(dataPrimeiroVenc), i - 1), { representation: 'date' });
    }
    // Status e datas
    ...
    // Para despesa simples, status sempre pago se vencimento <= hoje
    if (parcelas === 1 && dataVencimento <= hoje) {
      status = 'pago';
      dataPagamento = hoje;
    }
    if (status === 'pago') {
    ...
    const docRef = await addDoc(collection(db, "despesas_gerais"), {
      ...
      dataVencimento,
      dataPagamento,
      parcelaAtual: parcelas > 1 ? i : undefined,
      parcelaTotal: parcelas > 1 ? parcelas : undefined,
      groupId,
      data: dataVencimento // Para compatibilidade, data = dataVencimento
    });
    ids.push(docRef.id);
    await logAction(userId, 'create', 'despesas_gerais', docRef.id, `Despesa ${despesa.tipo} valor ${despesa.valor} (parcela ${i}/${parcelas})`);
  }
  return ids;
};
...
export const getResumoFinanceiro = async (
  ...
  despesasSnapshot.docs.forEach(doc => {
    const data = doc.data() as DespesaGeral;
    // Considera apenas despesas pagas para despesas operacionais
    const isPaga = data.status === 'pago' || !!data.dataPagamento;
    if (data.tipo === 'operacional' && isPaga) {
      const valor = (data.valor || 0);
      despesasOp += valor;
      listaDespesas.push({
        descricao: data.descricao || 'Despesa Operacional',
        valor: valor,
        data: data.dataPagamento || data.data,
        centroCustoId: data.centroCustoId
      });
  ...
  // Buscar despesas agendadas (a pagar) nos próximos 30 dias
  const hoje = new Date();
  ...
  // Filtrar despesas agendadas (status 'agendado' e dataVencimento entre hoje e +30d)
  const despesasAgendadas: Array<{ descricao: string; valor: number; dataVencimento: string }> = [];
  despesasSnapshot.docs.forEach(doc => {
    const data = doc.data() as DespesaGeral;
    if (
      data.status === 'agendado' &&
      data.dataVencimento &&
      data.dataVencimento >= hojeStr &&
      data.dataVencimento <= daqui30Str
    ) {
      despesasAgendadas.push({
        descricao: data.descricao || 'Despesa Agendada',
        valor: data.valor || 0,
